...
function getTeleinfo(): array
{
    ...
    } // on attend la fin d'une trame pour commencer à travailler avec la trame suivante
    ...
    foreach ($messages as $message) {
        $message = explode(
            ...
            $message,
            3
        ); // on separe l'etiquette, la valeur et la somme de controle de chaque message
...
function handlePuissance()
{
    ...
    $datas['hchp'] = substr(
        ...
        0,
        2
    ); // indicateur heure pleine/creuse, on garde seulement les carateres HP (heure pleine) et HC (heure creuse)
    $datas['va'] = preg_replace(
        '`^[0]*`',
        '',
        $trame['PAPP']
    ); // puissance en V.A, on supprime les 0 en debut de chaine
    $datas['iinst'] = preg_replace(
        '`^[0]*`',
        '',
        $trame['IINST']
    ); // intensité instantanée en A, on supprime les 0 en debut de chaine
...
function handleConso()
{
    ...
    $datas['total_hc'] = preg_replace(
        '`^[0]*`',
        '',
        $trame['HCHC']
    ); // conso total en Wh heure creuse, on supprime les 0 en debut de chaine
    $datas['total_hp'] = preg_replace(
        '`^[0]*`',
        '',
        $trame['HCHP']
    ); // conso total en Wh heure pleine, on supprime les 0 en debut de chaine
...
//
function getDatasPuissance($nb_days): string
{
    global $sqlite;
    $months = [
        '01' => 'janv',
    ...
    while ($row = $results->fetchArray(\SQLITE3_ASSOC)) {
        ...
        $datas[] = "[{v:new Date($year, $month, $day, $hour, $minute, $second), f:'".date(
            'j',
            $row['timestamp']
        ).' '.$months[date('m', $row['timestamp'])].' '.date(
            "H\hi",
            $row['timestamp']
        )."'}, {v:".$row['va'].", f:'".$row['va']." V.A'}, {v:".$row['watt'].", f:'".$row['watt']." kW'}]";
    }
